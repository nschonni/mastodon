...
class ImportService < BaseService
  ...
  def import_relationships!(action, undo_action, overwrite_scope, limit, extra_fields = {})
    ...
    items = @data.take(limit)
                 .map do |row|
              [row['Account address']&.strip&.delete_suffix(local_domain_suffix),
               extra_fields.to_h do |key, field_settings|
                 [key,
                  row[field_settings[:header]]&.strip || field_settings[:default]]
               end]
            end.reject { |(id, _)| id.blank? }
